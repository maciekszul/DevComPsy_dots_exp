...
for trial in range(n_trials):
    ...
    while True:
        ...
        if any(thr_bool):
            stim.feedback_update(correct, feedback)
            text_text = "Release the grip"
            info_text.text = text_text
            info_text.pos = (0,0)
            info_text.height = 2
            stim.draw([info_text, feedback])
            win.flip()
            ...
            update_json_file(calibration_file, {"tr_type_step_value": tr_type_step_value})
